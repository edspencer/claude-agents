...
2. **Changeset Evaluation**: Determine if a changeset phase is required using the decision framework in Check `.claude/docs/processes/changeset-management.md` (project) OR `~/.claude/plugins/repos/product-team/docs/processes/changeset-management.md` (plugin). Include a changeset phase if the plan modifies published packages (e.g., CLI).
